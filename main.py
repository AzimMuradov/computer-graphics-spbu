...
class MovingPointsCanvas(QGLWidget):
    ...
    def __init__(
        ...
        self.__points = np.random.uniform(-1, 1, size=(self.num_points, 2)).astype(
            np.float32
        )
        self.states = np.zeros(self.num_points, dtype=int)

        self.core = PointSystem(self.__points, self.r1, self.r2)
        ...
        self.target_positions = np.copy(self.__points)
    ...
    def points(self, value: np.ndarray):
        self.__points = value
        self.update_targets()
        self.core.points = value

    def generate_new_targets(self):
        while True:
            new_targets = self.__points + np.random.uniform(
                -self.speed_factor / 20, self.speed_factor / 20, self.__points.shape
            )
            yield new_targets
    ...
    def upate_states(self):
        self.core.apply_deltas(self.__points - self.target_positions)
    ...
    def initializeGL(self):
        ...
        glBufferData(
            GL_ARRAY_BUFFER, self.__points.nbytes, self.__points, GL_DYNAMIC_DRAW
        )
    ...
    def update_positions(self):
        ...
        self.__points += (self.target_positions - self.__points) * interpolation_speed

        # Update the VBO with new positions
        glBindBuffer(GL_ARRAY_BUFFER, self.VBO)
        glBufferSubData(GL_ARRAY_BUFFER, 0, self.__points.nbytes, self.__points)
...
class MainWindow(QMainWindow):
    def __init__(self, point_radius, num_points, image_path):
        ...
        self.speed_slider.setValue(10)  # Set default speed factor to 1.0 (scaled)
    ...
    def update_num_points(self, value):
        ...
        self.canvas.__points = np.random.uniform(-1, 1, size=(value, 2)).astype(
            np.float32
        )
    ...
    def update_speed(self, value):
        self.canvas.speed_factor = value / 10.0  # Adjust speed factor based on slider
